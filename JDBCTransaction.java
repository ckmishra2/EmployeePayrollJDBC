...
public class JDBCTransaction {
	public static void main(String[] args) {
		...
		try {
			...
			while (result.next()) {
				System.out.println("name: " + result.getString("name") + " Department: "
						+ result.getString("Department") + " salary: " + result.getInt("salary"));
			}
			// add new employee in payroll
			int sql = mystatement.executeUpdate(
					"insert into employee_payroll (name,salary,gender, start ) VALUES ('Elon', '900000','M', '2020-11-12')");
			...
			// Transaction step2: select and update salaries for Information technology
			result = mystatement.executeQuery(
					"SELECT name,salary from employee_payroll where Department = 'Information Technology'");
			...
			mystatement.executeUpdate(
					"update employee_payroll set salary= '4000' where Department = 'Information Technology'");
			result = mystatement.executeQuery(
					"SELECT name,salary from employee_payroll where Department = 'Information Technology'");
			while (result.next()) {
				System.out.println("name: " + result.getString("name") + " salary: " + result.getInt("salary"));
